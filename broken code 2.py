...
def check_serial_num():
    ...
    while flag:
        ...
        if len(ser_num) == 12 and ser_num.isdigit():
            ...
            print('Invalid serial number! It must be 12 digits long and contain only numbers.')
            flag = True
...
def record_job_reason():
        ...
        while flag:
            ...
            reason_choice = int(input('Enter reason choice here:  '))
            ...
            try:
                ...
                if local_choice <= 1  >= 5 and local_choice.is_integer:
...
def output_summary(name, sn, jr, jd, pjd, ts, jo, ns):
    ...
    with open('job_summaries.txt', 'w+') as job_summaries:
...
while True:
    ...
    if engineer_name.replace(" ", "").isalpha():
        break
    else:
